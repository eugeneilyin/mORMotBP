...
var
  ...
  // Allows to get MIME type associated with the specific file extension.
  // Typical use is as such:
  // !  ContentType :=
  // !    KnownMIMETypes.Value(ToUTF8(ExtractFileExt(FileName)), #0);
...
const
  ...
  //
  // The array below contains pairs of media type and related file extension.
  // File extensions are also commonly used to indicate content languages
  // and encodings (`*.json.gz` or `*.en.br`), so choose them carefully.
  //
  ...
  MIME_TYPES_FILE_EXTENSIONS: array[0..1490 shl 1 - 1] of RawUTF8 = (
    'application/a2l', '.a2l',
    'application/aml', '.aml',
...
begin
  ...
  ContentType :=
    KnownMIMETypes.Value(LowerCase(ToUTF8(ExtractFileExt(FileName))), #0);
  if ContentType = #0 then
    ContentType := 'application/octet-stream';
...
begin
  case Encoding of
    ...
    aeBrotli:
      begin
        BrotliExists := True;
        BrotliContent := AContent;
        BrotliHash := crc32c(0, Pointer(AContent), Length(AContent));
      end;
  end;
end;
...
begin
  Result := ForceDirectories(
    IncludeTrailingPathDelimiter(ExtractFilePath(FileName)));
  if not Result then Exit;
  Result := FileFromString(
    AlgoSynLZ.Compress(FAssetsDAH.SaveTo), FileName, True);
end;
...
begin
  ...
  for Index := 0 to Length(MIME_TYPES_FILE_EXTENSIONS) shr 1 - 1 do
    KnownMIMETypes.Add(MIME_TYPES_FILE_EXTENSIONS[Index shl 1 + 1],
